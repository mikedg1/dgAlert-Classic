...
public class ReminderReceiver extends BroadcastReceiver {
	 ...
	 * (>10 seconds) will cause a "Application Not Responding: Wait/Close"
	 * message to the user. Instead we'll fire up a service that does the work in
	 * a different thread.
	 */
	@Override
	...
	public static void scheduleReminder(Context context, SmsMmsMessage message) {
		...
		boolean reminder_notifications = myPrefs.getBoolean(context
		      .getString(R.string.pref_notif_repeat_key), Boolean.valueOf(context
		      .getString(R.string.pref_notif_repeat_default)));

		if (reminder_notifications) {
			int reminder_interval = Integer.parseInt(myPrefs.getString(context
			...
			reminderPendingIntent = PendingIntent.getBroadcast(context, 0, reminderIntent,
