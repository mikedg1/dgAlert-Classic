...
//Test
/*
 * This class provides an easy way to clear held WakeLocks and re-enable
 * the Keyguard (either immediately or scheduled in the future).
 */
public class ClearAllReceiver extends BroadcastReceiver {
	...
	/*
	 * Release WakeLocks and re-enable Keyguard (if reenableKeyguard is true).
	 * The only time we don't want to re-enable the Keyguard is if the user is
	 * trying to exit the Keyguard securely (exitKeyguardSecurely()) to perform
	 * an action like "Reply" or "Inbox".
	 */
...
//			ManageKeyguard.reenableKeyguard();
	...
	/*
	 * Cancels the scheduled Broadcast to this receiver (the user took some
	 * action so the Activity can now react to whatever they are doing).
	 */
